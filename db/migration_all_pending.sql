-- ============================================================================
-- Run this ONE file in the Supabase SQL Editor (SQL Editor → New query →
-- paste all of this → Run). It combines everything that's still outstanding
-- on the live database as of 2026-07-23: migration_001 (never applied) and
-- migration_002 (never applied). Idempotent — safe to run even if some
-- pieces already happened.
--
-- What this unblocks once it's run:
--   1. The "Request access" signup flow on the login page — currently
--      broken (queries a table, access_requests, that doesn't exist yet).
--   2. The Supabase Advisor security warnings from a few weeks ago — still
--      live right now, this is what actually fixes them.
--   3. Recap generation (manual "New Recap" AND the Fathom webhook) —
--      currently failing with "column clients.discord_channel_id does not
--      exist" because the Discord-bot delivery feature added that column
--      in code before the migration ran.
-- ============================================================================
...
-- ---------- 2. access_requests (signup gate) + ai_usage_log (if missing) ----------
...
-- ---------- 3. Discord bot delivery column ----------
alter table clients add column if not exists discord_channel_id text;

-- ---------- 4. Every RLS policy, recreated against private.* ----------
...
-- ---------- 5. Drop the old insecure public.* functions ----------
-- Only safe now that every policy above references private.* instead.
